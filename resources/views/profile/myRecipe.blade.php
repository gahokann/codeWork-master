...
<div class="content__recipe">
    <div class="recipe__start">
        <h3 class="recipe__title">Мои рецепты</h3>
        <a href="{{ route('profile.createRecipe', ['id' => $user->id])}}" class="btn btn-outline-secondary">Написать рецепт</a>
    </div>
    <form class="d-flex recipe__search" role="search">
        <input class="form-control me-2" id="inputSearch" type="search" placeholder="Поиск" aria-label="Поиск">
        <button onclick="tableSearch()" class="btn btn-outline-success" type="button">Поиск</button>
    </form>
        @if (session('status'))
        ...
        <table class="table align-middle" id="table">
          ...
          <tbody>
            ...
            <tr>
                ...
                <td><a href="{{ route('catalog.recipe', ['id'=>$val->id]) }}" class="btn btn-outline-primary">Открыть</a></td>
            </tr>
            @endforeach
            {{-- <tr>
                <th scope="row">1</th>
                ...
                <td>Математические функции</td>
                <td><time datetime="2019-04-29 19:00">2018-04-29</time></td>
                <td><a href="{{ route('catalog.recipe') }}" class="btn btn-outline-primary">Открыть</a></td>
            </tr> --}}
          </tbody>
        </table>
      </div>
